...
if(process.argv.length > 2){
  ...
  //can do stuff like: node ohmahgerd.js use "test.html index.html" css ".p" "color" "red"
  ...
  if(commands[0] === "open"){
    ...
    throw_err("ohmahgerd: " + commands[0] + ": command not defined. Try ohmahgerd --help to see all possible commands");
  }
  else{
    //no first command...
    if(tags.length > 0){
...
function findFiles(exp){ //like this: node ohmahgerd.js use "*.html" css ".p" "color" "red"
...
usage: ohmahgerd <mode> <files> [<command> <args>]

mode: use

  example: ohmahgerd use index.html add ".p" "<i>hi</i>" add ".p" "<b>also this</b>"
  example: ohmahgerd use "*.html" css ".p" "color" "red"
...
mode: open
  ...
  example: ohmahgerd open index.html
